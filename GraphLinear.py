...
# Things to enter
save_nm = 'Au_4-29'  # Save name to append
folder = 'Au_4-29-19'  # Folder name
vials = 7  # Number of ROIs
x = [0, 0.5, 1, 2, 3, 4, 5]  # Concentrations
...
plt.grid(False)
plt.xlabel("Iodine Concentration (%)", labelpad=15, fontsize=35)
plt.ylabel("Signal (HU)", labelpad=30, fontsize=35)
plt.title("a) Gold Signal Linearity", pad=50, fontsize=40)
...
for i in np.arange(6):
    ...
    if i < 3:
        ...
        ax[1, i - 3].set_ylim(-300, 2000)
        ax[1, i - 3].tick_params(labelsize=15)
...
plt.savefig(folder + '\AllBinsGraphs_' + save_nm + '.png', dpi=500)

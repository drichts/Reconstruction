...
class RedlenAnalyze(Analyze):
    def __init__(self, folder, mm='M20358_D32', type='UNIFORMITY', load_dir=r'X:\TEST LOG\MINI MODULE\Canon',
                 save_dir=r'C:\Users\10376\Documents\Phantom Data'):
        substring = os.path.join('Raw Test Data', mm, type)
        self.load_dir = os.path.join(load_dir, mm, folder, substring)
        self.save_dir = os.path.join(save_dir, folder)

        #super.__init__(folder, load_dir, save_dir)

    def testalign(self, folder, air_folder, num, directory='X:/TEST LOG/MINI MODULE/Canon/M20358_D32/'):
        """

...
class VisualizeUniformity:
    ...
    def blank_vs_time_six_bins(self, cnr_or_noise, test_num=0, pixel=1, end_time=100, save=False):
        """
        ...
        :param test_num: int, optional
                    The run number in the experiment, defaults to 0
        ...
        """
        px_idx = np.argwhere(self.AnalyzeUniformity.pxp == pixel)  # Find the index of the pixel value
        pixel_path = f'{pixel}x{pixel} Data'

        if cnr_or_noise == 1:
            cnr_vals = self.AnalyzeUniformity.cnr_time[test_num, px_idx]  # <files, pixels, bin, val, time>
            path = os.path.join(self.save_dir, 'Plots/CNR vs Time', pixel_path)
        else:
            cnr_vals = self.AnalyzeUniformity.noise_time[test_num, px_idx]  # <files, pixels, bin, val, time>
            path = os.path.join(self.save_dir, 'Plots/Noise vs Time', pixel_path)
        os.makedirs(path, exist_ok=True)

        frames = self.AnalyzeUniformity.frames
        titles = self.titles[test_num]

        end_idx = np.argwhere(frames == end_time) + 1  # The index of the last time desired on the plots
        frames = frames[0:end_idx]

        plot_cnr = np.zeros(11, 2, end_idx)
        plot_cnr[0:5] = cnr_vals[0:5, :, 0:end_idx]  # Get only SEC data up to the frame desired
        plot_cnr[6:11] = cnr_vals[6:11, :, 0:end_idx]  # Get only CC data up to the frame desired
        plot_cnr[11] = cnr_vals[12, :, 0:end_idx]  # Get EC bin for summed bin

        # Make some smooth data
        pc_shape = np.shape(plot_cnr)
        cnr_smth = np.zeros([pc_shape[0], pc_shape[2]])
        ...
        for i, ax in enumerate(axes.flatten()):
            if i < 5:
                ...
                ax.plot(frms_smth, cnr_smth[i], color='k')
    ...
    def blank_vs_pixels_six_bins(self, cnr_or_noise, test_num, time, save=False):

        titles = self.titles[test_num]
        if cnr_or_noise == 1:
            cnr_vals = self.AnalyzeUniformity.cnr_time[test_num]  # <files, pixels, bin, val, time>
            path = os.path.join(self.save_dir, 'Plots/CNR vs Time')
        else:
            cnr_vals = self.AnalyzeUniformity.noise_time[test_num]  # <files, pixels, bin, val, time>
            path = os.path.join(self.save_dir, 'Plots/Noise vs Time')
        os.makedirs(path, exist_ok=True)

        pixels = self.AnalyzeUniformity.pxp
        time_idx = np.argwhere(self.AnalyzeUniformity.frames == time)
        ...
        plot_cnr = np.zeros(11, 2, len)
        plot_cnr[0:5] = cnr_vals[0:5]  # Get only SEC data up to the frame desired
        plot_cnr[6:11] = cnr_vals[6:11]  # Get only CC data up to the frame desired
        plot_cnr[11] = cnr_vals[12]  # Get EC bin for summed bin

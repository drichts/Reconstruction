...
directory1 = r'X:\TEST LOG\MINI MODULE\Canon\M20358_Q20/multiple_energy_thresholds_flatfield_3w/'
directory2 = r'X:\TEST LOG\MINI MODULE\Canon\M20358_Q20/multiple_energy_thresholds_3w/'
path1 = directory1 + '/Raw Test Data/M20358_Q20/UNIFORMITY/*A0*'
path2 = directory2 + '/Raw Test Data/M20358_Q20/UNIFORMITY/*A0*'
files1 = glob.glob(path1)
files2 = glob.glob(path2)

airdata = loadmat(files1[0])
scan_data = loadmat(files1[1])

data = np.squeeze(airdata['cc_struct']['data'][0][0][0][0][0])
air = np.squeeze(airdata['cc_struct']['data'][0][0][0][0][0])
data = np.sum(data, axis=1)
air = np.sum(air, axis=1)

corr = -1*np.log(np.divide(data, air))

print(np.nanstd(corr[7, 2:20, 2:34])/np.nanmean(corr[7, 2:20, 2:34]))

def gogo(b):

    datafull = np.array(scan_data['cc_struct']['data'][0][0][0][0][0])
    airfull = np.array(airdata['cc_struct']['data'][0][0][0][0][0])
    ...
    dif2 = np.nanmean(data2*bg)
    std2 = np.nanstd(data2*bg)
    dif5 = np.nanmean(data*bg)
    std5 = np.nanstd(data*bg)
    dif1 = np.nanmean(datafull*bg)
    std1 = np.nanstd(datafull*bg)
...
def gogo4(b):
    ...
    data = np.squeeze(np.array(scan_data['cc_struct']['data'][0][0][0][0][0]))[b]
    air = np.squeeze(np.array(airdata['cc_struct']['data'][0][0][0][0][0]))[b]
    ...
    for i in np.arange(500):
        new_data = -1*np.log(np.divide(data[i], air[i]))
        dif001[i] = np.nanmean(new_data * mask) - np.nanmean(new_data*bg)
        mean001[i] = np.nanmean(new_data * bg)
        std001[i] = np.nanstd(new_data * bg)
    ...
    data = np.sum(data, axis=0)
    air = np.sum(air, axis=0)
    data5 = np.sum(data5, axis=0)
    air5 = np.sum(air5, axis=0)
    data01 = np.sum(data01, axis=0)
    air01 = np.sum(air01, axis=0)

    data = -1*np.log(np.divide(data, air))
    data5 = -1 * np.log(np.divide(data5, air5))
    data01 = -1 * np.log(np.divide(data01, air01))
